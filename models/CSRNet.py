...
class CSRNet(nn.Module):
    ...
    def forward(self, label_x):
        ...
        print(label_x.shape)#VGG BACKBONE

        label_x = self.backend(label_x)
        print(label_x.shape)# REVERSE BACKBONE LIKE
        label_x = self.output_layer(label_x)
        print(label_x.shape)# KEEP MINI IMAGESIZE
        label_x = F.interpolate(label_x, scale_factor=8, mode='bilinear', align_corners=True)
        print(label_x.shape)#ORIGIN IMAGE SIZE
        return label_x
...
if __name__ == '__main__':
    input_demo = torch.rand((2,3,224,224))
    target_demo = torch.rand((2,1,224,224))
