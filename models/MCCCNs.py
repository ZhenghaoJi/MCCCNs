...
class MCCCN(nn.Module):
    ...
    def forward(self, x):
        ...
        print(sum2_upsamples.shape)
        sum1 = self.conv1_scale(torch.cat((sum1, sum2_upsamples), 1))
        sum1_upsamples =F.interpolate(sum1, sum0.shape[2:], mode='bilinear',align_corners=True)
        print(sum1_upsamples.shape)
        sum0 = self.conv0_scale(torch.cat((sum0, sum1_upsamples), 1))
        sum0_upsamples =F.interpolate(sum0, x.shape[2:], mode='bilinear',align_corners=True)
